...
public class RsStreamTextureRendererAndConverter : MonoBehaviour
{
    ...
    public float convertThreshould = 0.22f;

    protected Texture2D texture;

    private Texture2D rosTex;
    ...
    /*private Texture2D TextureConvert()
    {
        Color[] colors = texture.GetPixels();
        //Debug.Log("colors:" + colors.Length);
        for(int i = 0; i < colors.Length; i++)
        {
            float maxValue = Mathf.Max(colors[i].r, Mathf.Max(colors[i].g, colors[i].b));
            float minValue = Mathf.Min(colors[i].r, Mathf.Min(colors[i].g, colors[i].b));
            float s = (maxValue - minValue) / maxValue;

            if(s < convertThreshould)
            {
                colors[i].r = 1f;
                colors[i].g = 1f;
                colors[i].b = 1f;
        ...
        Texture2D returnValue = new Texture2D(texture.width, texture.height, texture.format, false, true);
        returnValue.SetPixels(colors);
        return returnValue;
    }*/
